...
function setTableBody() {
    ...
    for (let i = 0; i < storage.orders.length; i++) {
        ...
        //Ligne produit remplie avec les éléments de chaque article
        $tr.innerHTML +=
        ...
        $trash.addEventListener("click", () => {
            ...
            if (confirm("Voulez-vous vraiment supprimer l'article ?")) {
                ...
                if (totalQuantity === 0 && totalOrder === 0) {
                    deleteCartAndForm();
                }
            }
        });
...
function setTableFooter(totalQuantity, totalOrder) {
...
//Exécution des 3 fonctions du tableau

setTableHeader();
setTableBody();
setTableFooter(totalQuantity, totalOrder);
...
//Actions dues à la suppression du panier
const deleteCartAndForm = () => {
...
// FORMULAIRE ---------------------------------------------------------------------------
const $h2 = document.createElement("h2"); //Titre du formulaire
const $form = document.createElement("form") //Formulaire
...
function createForm() {
    ...
    $emailField.setAttribute("pattern", "[a-zA-Z0-9._%+-]+@[a-zA-Z0-9._-]+\.[a-zA-Z]{2,15}$");
    ...
    //Focus sur le 1er champ à remplir (1er bouton radio)
    ...
    $form.addEventListener("submit", (e) => {
        e.preventDefault(); //empêche la validation par défault
        ...
        //Stocke en local le montant de la commande pour la confirmation
        const $totalAmount = document.querySelector("#totalAmount");
...
//Le formulaire ne s'affiche que s'il y a des produits dans le panier, sinon il n'apparait pas et indique que le panier est vide
